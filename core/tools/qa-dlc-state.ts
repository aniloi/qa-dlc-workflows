#!/usr/bin/env bun
// qa-dlc-state.ts — the tool-owned session state (aidlc-docs/qa-state.md).
//
...
// never drift. The engine (qa-dlc-orchestrate.ts) reads/writes it; the conductor
// never hand-edits the machine fields.
//
// CLI:
//   bun qa-dlc-state.ts show                    print the JSON state (or {} if none)
//   bun qa-dlc-state.ts init --scope <s> --depth <d> [--story-source <m>]
//   bun qa-dlc-state.ts set --field K=V ...     patch scalar fields
//   bun qa-dlc-state.ts complete --stage <slug> mark a stage done
//
...
function main(): void {
  ...
  process.stderr.write("usage: qa-dlc-state.ts <show|init|set|complete> [flags]\n");
  process.exit(2);
}
