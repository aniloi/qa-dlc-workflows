#!/usr/bin/env bun
// qa-dlc-audit.ts — the append-only audit trail. Every user input and every
// engine/hook event lands here as a timestamped block. NEVER overwrites; only
// appends. Mirrors QA-DLC v1's audit.md format.
//
// CLI:
//   bun qa-dlc-audit.ts append --event <TYPE> [--field K=V ...]
//   bun qa-dlc-audit.ts init                    create the audit file if absent
//
...
function main(): void {
  ...
  process.stderr.write("usage: qa-dlc-audit.ts <init|append> [--event T] [--field K=V ...]\n");
  process.exit(2);
}
