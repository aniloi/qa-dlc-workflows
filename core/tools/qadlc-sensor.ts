#!/usr/bin/env bun
// qadlc-sensor.ts — the sensor DISPATCHER. Given a stage and a written file, it
...
// hand:
//
//   bun qadlc-sensor.ts --stage <slug> --file-path <path>
//
...
function main(): void {
  ...
  if (!stage || !filePath) {
    process.stderr.write("usage: qadlc-sensor.ts --stage <slug> --file-path <path>\n");
    process.exit(2);
  }
