...
- **The engine** (`{{QADLC_CMD}}`) owns routing:
...
- **An unreachable engine ends the run — it is not a licence to improvise.** If
  a `{{QADLC_CMD}}` call fails (`command not found`, a non-zero exit, an
  ...
  silently switched off. Run `{{PREFLIGHT_CMD}}` to confirm the cause.
