...
|---|---|---|
| A standing preference ("always tag API scenarios `@api`") | `{{HARNESS_DIR}}/memory/team.md` (or `project.md` if repo-specific) | Loaded before every stage; wins over repo-derived conventions |
| A repeatable check ("no scenario may exceed 12 steps") | a new `{{HARNESS_DIR}}/sensors/qa-dlc-<id>.md` manifest + a `qa-dlc-sensor-<id>.ts` tool + the `<id>` added to a stage's `sensors:` list | Fires automatically on matching file writes |
...
(`bun {{HARNESS_DIR}}/tools/qa-dlc-graph.ts compile`) so the new binding takes
