...
| A repeatable check ("no scenario may exceed 12 steps") | a new `{{HARNESS_DIR}}/sensors/qadlc-<id>.md` manifest + a `qadlc-sensor-<id>.ts` tool + the `<id>` added to a stage's `sensors:` list | Fires automatically on matching file writes |
...
(`bun {{HARNESS_DIR}}/tools/qadlc-graph.ts compile`) so the new binding takes
