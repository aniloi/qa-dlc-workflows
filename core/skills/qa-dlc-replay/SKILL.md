---
name: qa-dlc-replay
description: Replay a QA-DLC session from the audit trail — reconstruct the sequence of stages, approvals, feature writes, and sensor findings for review or recovery.
---
...
3. Cross-check against current state
   (`bun {{HARNESS_DIR}}/tools/qa-dlc-state.ts show`) and flag any divergence
