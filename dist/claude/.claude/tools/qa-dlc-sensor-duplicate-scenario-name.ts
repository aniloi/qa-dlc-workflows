#!/usr/bin/env bun
// qa-dlc-sensor-duplicate-scenario-name.ts — detect scenario names that collide
...
//
//   bun qa-dlc-sensor-duplicate-scenario-name.ts --stage <slug> --file-path <path>
...
function main(): void {
  const { filePath } = parseArgs(process.argv.slice(2), "qa-dlc-sensor-duplicate-scenario-name");
