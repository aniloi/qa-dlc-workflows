#!/usr/bin/env bun
// qa-dlc-graph.ts — the stage-graph compiler.
//
//   bun qa-dlc-graph.ts compile            (re)compile into tools/data/*.json
//   bun qa-dlc-graph.ts compile --check    verify committed JSON is fresh
//
...
} from "./qa-dlc-stage-schema.ts";
import { parseScope, validateScope, type Depth } from "./qa-dlc-scope-schema.ts";
import { parseSensorManifest, validateSensorManifest } from "./qa-dlc-sensor-schema.ts";
...
export function compileGraph(root: string): { stageGraph: StageGraph; scopeGrid: ScopeGrid } {
  ...
  for (const file of listMarkdown(scopesDir)) {
    const raw = readFileSync(file, "utf-8");
    const name = stemAfterPrefix(file, "qa-dlc-");
  ...
  for (const file of listMarkdown(sensorsDir)) {
    const raw = readFileSync(file, "utf-8");
    const id = stemAfterPrefix(file, "qa-dlc-");
...
function main(): void {
  ...
  if (cmd !== "compile") {
    process.stderr.write("usage: qa-dlc-graph.ts compile [--check]\n");
    process.exit(2);
  }
  if (check) {
