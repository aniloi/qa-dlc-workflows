...
- **The engine** (`.claude/tools/qadlc-orchestrate.ts`) owns routing:
...
- **An unreachable engine ends the run — it is not a licence to improvise.** If
  ...
  `sh .claude/tools/qadlc-preflight.sh` to confirm the cause.
...
(`.claude/agents/<lead_agent>.md`) and adopt its voice for the stage
