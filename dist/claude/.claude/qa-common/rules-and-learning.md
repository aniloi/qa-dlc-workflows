...
| A repeatable check ("no scenario may exceed 12 steps") | a new `.claude/sensors/qa-dlc-<id>.md` manifest + a `qa-dlc-sensor-<id>.ts` tool + the `<id>` added to a stage's `sensors:` list | Fires automatically on matching file writes |
...
(`bun .claude/tools/qa-dlc-graph.ts compile`) so the new binding takes
