...
|---|---|---|
| A standing preference ("always tag API scenarios `@api`") | `/memory/team.md` (or `project.md` if repo-specific) | Loaded before every stage; wins over repo-derived conventions |
| A repeatable check ("no scenario may exceed 12 steps") | a new `/sensors/qadlc-<id>.md` manifest + a `qadlc-sensor-<id>.ts` tool + the `<id>` added to a stage's `sensors:` list | Fires automatically on matching file writes |
...
(`bun /tools/qadlc-graph.ts compile`) so the new binding takes
