...
knowledge dir under `/knowledge/<agent>/` if present, (3) team
memory under `/memory/` (team rule wins on conflict), and (4) the
...
- **Prescriptive rule** → write to team memory (`/memory/team.md`
  or `project.md`). Next run loads it automatically.
- **Verification check** → author a new sensor manifest at
  `/sensors/qadlc-<id>.md` and add `<id>` to the relevant stage's
  `sensors:` list.
