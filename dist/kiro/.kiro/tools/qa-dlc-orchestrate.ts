#!/usr/bin/env bun
// qa-dlc-orchestrate.ts — the deterministic ENGINE.
//
...
//
//   bun qa-dlc-orchestrate.ts next
//       → prints one JSON directive (detect-scope | run-stage | gate | done)
//   bun qa-dlc-orchestrate.ts report <flags>
...
import type { StageGraph, ScopeGrid } from "./qa-dlc-graph.ts";
import type { StageDefinition } from "./qa-dlc-stage-schema.ts";
...
function next(): Directive {
  ...
  if (!state || !state.scope) {
    return {
      type: "detect-scope",
      message:
        "No active QA-DLC session. Detect the scope from the user's request " +
        "(match keywords), confirm it, then run: " +
        "qa-dlc-orchestrate.ts report --scope <name> [--depth <d>] [--story-source <mode>].",
  ...
  if (remaining.length === 0) {
    return { type: "done", message: `QA-DLC complete for scope "${state.scope}". All stages done.` };
  }
...
function main(): void {
  ...
  process.stderr.write("usage: qa-dlc-orchestrate.ts <next|report> [flags]\n");
  process.exit(2);
}
