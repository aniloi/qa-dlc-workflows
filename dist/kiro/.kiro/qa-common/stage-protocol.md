...
knowledge dir under `.kiro/knowledge/<agent>/` if present, (3) team
memory under `.kiro/memory/` (team rule wins on conflict), and (4) the
...
- **Prescriptive rule** → write to team memory (`.kiro/memory/team.md`
  or `project.md`). Next run loads it automatically.
- **Verification check** → author a new sensor manifest at
  `.kiro/sensors/qa-dlc-<id>.md` and add `<id>` to the relevant stage's
  `sensors:` list.
