...
- **The engine** (`bun .kiro/tools/qadlc.ts`) owns routing:
...
- **An unreachable engine ends the run — it is not a licence to improvise.** If
  a `bun .kiro/tools/qadlc.ts` call fails (`command not found`, a non-zero exit, an
  ...
  silently switched off. Run `sh .kiro/tools/qadlc-preflight.sh` to confirm the cause.
