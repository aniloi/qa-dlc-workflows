---
name: qadlc
description: QADLC Gherkin feature-file workflow. Trigger on "Using QADLC", or on keywords BDD, Gherkin, feature file. Plan-first, two-phase, deterministic engine-driven.
---
...
1. Call `bun .kiro/tools/qadlc-orchestrate.ts next` to get the next directive.
...
3. Do exactly the one move the directive names (run a stage, ask a question,
   present a gate).
4. Call `bun .kiro/tools/qadlc-orchestrate.ts report …` with the outcome.
...
> Phase 1 scaffold: the engine (`qadlc-orchestrate.ts`) and stage files are
