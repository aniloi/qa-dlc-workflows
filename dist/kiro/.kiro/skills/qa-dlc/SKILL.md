...
1. Call `bun .kiro/tools/qa-dlc-orchestrate.ts next` to get the next directive.
...
3. Do exactly the one move the directive names (run a stage, ask a question,
   present a gate).
4. Call `bun .kiro/tools/qa-dlc-orchestrate.ts report …` with the outcome.
