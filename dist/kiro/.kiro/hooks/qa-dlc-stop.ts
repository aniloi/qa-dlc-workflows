#!/usr/bin/env bun
// qa-dlc-stop.ts — Stop hook. The ENFORCEMENT hook: it turns QA-DLC's two hard
...
import { appendAuditEntry } from "../tools/qa-dlc-audit.ts";
...
try {
  ...
  if (!planApproved && (featureCreatedIdx >= 0 || state.feature_files_written > 0)) {
    ...
    block(
      "QA-DLC plan gate: a .feature file was created before the Gherkin Plan was " +
        "approved. Present gherkin_plan.md, get explicit approval, and report it " +
        "(qa-dlc-orchestrate.ts report --stage gherkin-plan --approved) before " +
  ...
  if (featureBeforeApproval) {
    ...
    block("QA-DLC plan gate: a feature file appears in the audit before plan approval. Review the order of operations.");
  }
  ...
  if (existsSync(plan) && state.feature_files_written > 0) {
    ...
    if (checked < state.feature_files_written) {
      process.stdout.write(
        `QA-DLC checkbox notice: ${state.feature_files_written} feature file(s) written but only ` +
