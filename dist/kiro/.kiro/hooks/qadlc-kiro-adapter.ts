#!/usr/bin/env bun
// qadlc-kiro-adapter.ts — the Kiro hook seam.
//
...
//
//   bun .kiro/hooks/qadlc-kiro-adapter.ts --hook <core-hook-name>
//
// e.g. --hook qadlc-audit-logger. Keeps the "one core, N harnesses" promise:
// the hook logic lives in .kiro/hooks/qadlc-*.ts (copied from core); only this
...
if (!hook) {
  process.stderr.write("usage: qadlc-kiro-adapter.ts --hook <core-hook-name>\n");
  process.exit(2);
}
